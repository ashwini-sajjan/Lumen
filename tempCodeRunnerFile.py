...
def type_like_ide(text):
    """Type code exactly like an IDE would - with smart auto-completion and formatting"""
    ...
    print(f"Starting to type in {typing_delay} seconds... Click in your IDE where you want the code")
    ...
    for i, line in enumerate(lines):
        ...
        # Type character by character for IDE auto-completion
        for char in content:
            pyautogui.write(char, interval=typing_speed)
            
            # Handle IDE auto-completion triggers
            if char in '({[':
                time.sleep(0.02)  # Let IDE show suggestions
            elif char == '.' and language in ['cpp', 'java', 'javascript']:
                time.sleep(0.05)  # Wait for method suggestions
            elif char == ':' and language == 'python':
                time.sleep(0.02)  # Python colon handling
        
        # Handle end of line
        if i < len(lines) - 1:
            pyautogui.press('enter')
            time.sleep(0.05)  # Let IDE auto-indent
            
            # Handle special cases for next line preparation
            if content.endswith('{') or content.endswith(':'):
                time.sleep(0.1)  # Extra time for IDE to process indentation
